...
struct MediaInfoPanelView: View {
    ...
    private var parsedSections: [MediaInfoPanelSection] {
        MediaInfoPanelTextParser.parse(controller.rawText)
    }
        ...
        HStack(spacing: 10) {
            ...
            HStack(spacing: 12) {
                ...
                navButton(
                    ...
                    disabled: controller.currentIndex >= controller.mediaFiles.count - 1)
            }
        }
                        ...
                        ForEach(section.rows) { row in
                            HStack(alignment: .firstTextBaseline, spacing: 8) {
                                if row.label.isEmpty {
                                    ...
                                    Text(row.value)
                                        .font(.system(size: 13))
                                        .foregroundStyle(valueColor(for: row.valueStyle))
    ...
    private func valueColor(for style: MediaInfoPanelRow.ValueStyle) -> Color {
        switch style {
        case .path:
    ...
    private var convertCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if controller.isConvertible {
                ...
                row("Output") {
                    DialogTextField("Filename", text: $controller.outputName)
