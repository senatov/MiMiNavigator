...
struct FileTableView: View {
    ...
    var body: some View {
        ...
        return ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                }.padding(.vertical, 4).padding(.horizontal, 6).background(Color(nsColor: .windowBackgroundColor)).overlay(
        ...
        }.frame(maxWidth: .infinity, maxHeight: .infinity).padding(.horizontal, 6).overlay(
    ...
    private func getNameColSortableHeader() -> some View {
        ...
        return HStack(spacing: 4) {
            ...
            if sortKey == .name { Image(systemName: sortAscending ? "arrowtriangle.up.fill" : "arrowtriangle.down.fill").font(.caption2) }
    ...
    private func getSizeColSortableHeader() -> some View {
        ...
        return HStack(spacing: 4) {
            ...
            if sortKey == .size { Image(systemName: sortAscending ? "arrowtriangle.up.fill" : "arrowtriangle.down.fill").font(.caption2) }
        }.frame(width: FilePanelStyle.sizeColumnWidth, alignment: .leading).contentShape(Rectangle()).onTapGesture {
            appState.focusedPanel = panelSide
    ...
    private func getDateSortableHeader() -> some View {
        ...
        return HStack(spacing: 4) {
            ...
            if sortKey == .date { Image(systemName: sortAscending ? "arrowtriangle.up.fill" : "arrowtriangle.down.fill").font(.caption2) }
        }.frame(width: FilePanelStyle.modifiedColumnWidth + 10, alignment: .leading).contentShape(Rectangle()).onTapGesture {
            appState.focusedPanel = panelSide
    ...
    @ViewBuilder private func highlightedSquare(_ isSel: Bool) -> some View {
        if isSel { Rectangle().inset(by: 0.2).stroke(FilePanelStyle.blueSymlinkDirNameColor.gradient, lineWidth: 1.0) } else { EmptyView() }
    }
    ...
    private func drawFileLineInTheTable(_ index: Int, _ isSel: Bool, _ file: CustomFile) -> some View {
        return ZStack(alignment: .leading) {
            ...
            if isSel { Rectangle().fill(FilePanelStyle.yelloeSelectedRowFill).allowsHitTesting(false) }
            rowContent(file: file, isSel: isSel)
        }.frame(maxWidth: .infinity, alignment: .leading).contentShape(Rectangle()).highPriorityGesture(
            TapGesture().onEnded {
                selectedID = file.id
                onSelect(file)
            }
        ).overlay(highlightedSquare(isSel)).shadow(color: isSel ? .black.opacity(0.2) : .clear, radius: 4, x: 0, y: 2).animation(
            .spring(response: 0.4, dampingFraction: 0.7), value: isSel)
    }
    ...
    @ViewBuilder private func rowContent(file: CustomFile, isSel: Bool) -> some View {
        HStack(alignment: .center, spacing: 8) {
            // Name column (expands)
            FileRowView(file: file, isSelected: isSel).frame(maxWidth: .infinity, alignment: .leading)
            // vertical separator
            Rectangle().frame(width: 1).foregroundColor(Color.secondary.opacity(0.15)).padding(.vertical, 2)
            // Size column
            Text(file.fileObjTypEnum).foregroundColor(Color(#colorLiteral(red: 0.1215686277, green: 0.01176470611, blue: 0.4235294163, alpha: 1))).frame(
                width: FilePanelStyle.sizeColumnWidth, alignment: .leading)
            // vertical separator
            Rectangle().frame(width: 1).foregroundColor(Color.secondary.opacity(0.15)).padding(.vertical, 2)
            // Date column
            Text(file.modifiedDateFormatted).foregroundColor(Color(#colorLiteral(red: 0.3098039329, green: 0.01568627544, blue: 0.1294117719, alpha: 1)))
