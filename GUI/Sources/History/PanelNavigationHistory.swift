    ...
    final class PanelNavigationHistory {
        ...
        func navigateTo(_ url: URL) {
            ...
            save()
            log.info("\(#function) panel=\(panel) added: \(normalized.path.components(separatedBy: "/").suffix(3).joined(separator: "/")) index=\(currentIndex)/\(history.count)")
        }
        ...
        func goBack() -> URL? {
            ...
            if !history.isEmpty {
                log.debug("\(#function) history: \(history.enumerated().map { "\($0.offset): \($0.element.path.components(separatedBy: "/").suffix(2).joined(separator: "/"))" })")
            }
