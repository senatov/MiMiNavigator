// ToolbarCustomizeSections.swift
// MiMiNavigator
//
...
// MARK: - Toolbar Customize Sections
struct TCV_Header: View {
    var body: some View {
        HStack(spacing: 8) {
            Spacer()
            ...
            HStack(spacing: 6) {
                ForEach(store.visibleItems) { item in
                    TCV_PreviewButton(item: item, isDragging: dragPreview == item)
                        ...
                        .onDrop(
                            of: [.text],
                            delegate: PreviewReorderDelegate(
                                targetItem: item,
                                store: store,
                ...
                if dragPool != nil {
                    RoundedRectangle(cornerRadius: 6)
                        ...
                        .onDrop(
                            of: [.text],
                            delegate: PreviewAppendDelegate(
                                store: store,
                                dragPool: $dragPool
                            ))
                }
...
struct TCV_Palette: View {
    ...
    private var items: [ToolbarItemID] {
        ...
        return all.filter { $0.label.lowercased().contains(query) || $0.helpText.lowercased().contains(query) }
    }
...
struct TCV_PaletteCell: View {
    ...
    var body: some View {
        ...
        .help(
            item.isFixed ? "Always in toolbar (cannot be removed)" : isLastOne ? "At least one button must be visible" : item.helpText
        )
        .animation(.easeInOut(duration: 0.15), value: isVisible)
    }
}

struct TCV_Footer: View {
    let store: ToolbarStore
    let onReset: () -> Void

    var body: some View {
        HStack {
            DownToolbarButtonView(title: "Reset", systemImage: "arrow.counterclockwise", action: onReset)
            Spacer()
