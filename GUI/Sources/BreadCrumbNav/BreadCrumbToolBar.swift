...
struct BreadCrumbToolBar: View {
    ...
    private func handleNavigateUp() {
        ...
        let parentPath = resolvedParentPath(for: currentURL)

        Task {
            await navigator.navigate(to: parentPath, side: panelSide)
        }
    }

    private func resolvedParentPath(for currentURL: URL) -> String {
