...
final class MiMiDefaults {
    ...
    private init() {
        ...
        fileURL = dirURL.appendingPathComponent("defaults.json")
        ensureDirectory()
        load()
        migrateFromUserDefaultsIfNeeded()
        subscribeTermination()
    }
    // MARK: - Getters (matching UserDefaults API surface)
    ...
    private static let migrationDoneKey = "MiMiDefaults.migratedFromUserDefaults"
    private func migrateFromUserDefaultsIfNeeded() {
        guard !storage.keys.contains(Self.migrationDoneKey) else { return }
        ...
        // Migrate well-known non-PreferenceKeys keys used by ToolbarStore, etc.
        let extraKeys = [
            "MiMi.toolbarOrder", "MiMi.toolbarVisible", "MiMi.menuBarVisible",
            "MiMi.openWithLRU", "MiMi.openWithAppURLs",
        ...
        storage[Self.migrationDoneKey] = true
        if migrated > 0 {
