...
final class SplitContainerCoordinator<Left: View, Right: View>: NSObject, NSSplitViewDelegate {
    ...
    func splitView(
        ...
        log.debug(#function)
        // Respect system proposal, but never allow left panel->be smaller than our minimum
    ...
    func splitView(
        ...
        log.debug(#function)
        // Ensure right panel also respects minimum width when divider moves right
    ...
    @objc func handleDoubleClickFromSplitView(_ sv: NSSplitView) {
        log.debug(#function + " — resetting to 50/50")
