...
final class PanelStartupCache: @unchecked Sendable {
    ...
    private struct CustomFileSnapshot: Codable {
        let path: String
    }
    ...
    private struct CachePayload: Codable {
        ...
        let leftFiles: [CustomFileSnapshot]
        let rightFiles: [CustomFileSnapshot]
        let savedAt: Date
    }
    ...
    func save(
        ...
        let payload = CachePayload(
            leftPath: leftPath,
            rightPath: rightPath,
            leftFiles: filteredLeft.map { CustomFileSnapshot(path: $0.urlValue.path) },
            rightFiles: filteredRight.map { CustomFileSnapshot(path: $0.urlValue.path) },
            savedAt: Date()
        )
        do {
    ...
    func load(forLeftPath leftPath: String, rightPath: String) -> (left: [CustomFile], right: [CustomFile])? {
        ...
        do {
            ...
            let left = payload.leftFiles.map { CustomFile(path: $0.path) }
            let right = payload.rightFiles.map { CustomFile(path: $0.path) }
            return (left, right)
        } catch {
