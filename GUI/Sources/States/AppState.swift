...
@MainActor final class AppState: ObservableObject {
    ...
    @MainActor func select(_ file: CustomFile, on side: PanelSide) {
        focusedPanel = side
        switch side {
            case .left:
        ...
        /// Clear selection on the given side without affecting focus.
    @MainActor func clearSelection(on side: PanelSide) {
        switch side {
        ...
        /// Get currently selected file for the given side.
    @MainActor func selectedFile(on side: PanelSide) -> CustomFile? {
        switch side {
