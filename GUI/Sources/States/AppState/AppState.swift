...
final class AppState {
    ...
    /// 1-based index of first visible row during scrolling (approximate)
    ...
    /// Get visible index for panel (first visible row during scroll)
    ...
    /// Set visible index for panel (called from FileTableView while scrolling)
    ...
    init() {
        ...
        // Restore sorting preferences from previous session
...
extension AppState {
    ...
    func refreshAndSelectAfterRemoval(removedFiles: [CustomFile], on panel: PanelSide) async {
        ...
        log.debug(
            "[AppState] refreshAndSelectAfterRemoval: lastRemovedIndex=\(lastRemovedIndex) oldFiles=\(oldFiles.count) panel=\(panel)")
        if panel == .left {
            await refreshLeftFiles()
        } else {
            await refreshRightFiles()
        ...
        log.info(
            "[AppState] refreshAndSelectAfterRemoval on \(panel) → index \(targetIndex) '\(targetFile.nameStr)' (of \(newFiles.count))"
        )
    }
...
extension AppState {
    ...
    func updateSorting(key: SortKeysEnum? = nil, ascending: Bool? = nil) {
        ...
        // Persist sorting between sessions
