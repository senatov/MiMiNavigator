...
//  MiMiNavigator
//
//  Created by Iakov Senatov on 27.09.24.
//
...
extension AppStorage where Value == String {
  init(
    _ preferenceKeyPath: KeyPath<UserPreferences, UserPreferences.Preference<Value>>,
    store: UserDefaults? = nil
  ) {
    let preference = UserPreferences.shared[keyPath: preferenceKeyPath]

    self.init(
      wrappedValue: preference.defaultValue,
      preference.key,
      store: store)
  }
}
...
class UserDefault<Value> {
  ...
  where Value: RawRepresentable {
    ...
    getter = {
      guard let rawValue = self.container.object(forKey: self.key) as? Value.RawValue else {
  ...
  where Value == R?, R: RawRepresentable {
    ...
    getter = {
      guard let rawVal = self.container.object(forKey: self.key) as? R.RawValue else {
        return self.defaultValue
      }
