...
actor DualDirectoryScanner: ObservableObject {
    ...
    func startMonitoring() {
        LoggerManager.log.info("startMonitoring()")
        setupTimer(for: .left)
        setupTimer(for: .right)
        if leftTimer == nil || rightTimer == nil {
            LoggerManager.log.error("Failed to initialize one or both timers.")
        }
    }

    // MARK: - Helper method to setup timers
    private func setupTimer(for side: DirectorySide) {
        LoggerManager.log.info("setupTimer() \(side)")
    ...
    func stopMonitoring() {
        LoggerManager.log.info("stopMonitoring()")
        leftTimer?.cancel()
        leftTimer = nil
        rightTimer?.cancel()
        rightTimer = nil
    ...
    private func refreshFiles(for side: DirectorySide) async {
        LoggerManager.log.debug("refreshFiles() for <<\(side)>> directory.")
        ...
        do {
            ...
            LoggerManager.log.error("Failed to scan \(side) directory: \(error.localizedDescription)")
            files = []
        }
    ...
    private func updateFileList(for side: DirectorySide, with files: [CustomFile]) async {
        LoggerManager.log.info("updateFileList()")
        switch side {
        case .left:
            await fileLst.updateLeftFiles(files)
            LoggerManager.log.info("Left directory updated with \(files.count) files.")
        case .right:
            await fileLst.updateRightFiles(files)
            LoggerManager.log.info("Right directory updated with \(files.count) files.")
        }
    }

    // MARK: - Scans a directory for files and directories
    private func scanDirectory(at url: URL?) async throws -> [CustomFile] {
        LoggerManager.log.info("scanDirectory() dir: \(String(describing: url?.path))")
        guard let url = url else {
            LoggerManager.log.error("Invalid directory URL: URL is nil.")
            return []
        }
        ...
        do {
            ...
            LoggerManager.log.error("Failed to scan directory at: \(url.path): \(error.localizedDescription)")
            throw error
        }
        return customFiles
    }
