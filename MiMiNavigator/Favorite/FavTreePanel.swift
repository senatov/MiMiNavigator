...
struct FavTreePanel {
        ...
        do {
            ...
            return contents.filter {
                guard let values = try? $0.resourceValues(forKeys: [networkKey]),
                    let isNetwork = values.allValues[networkKey] as? Bool
                else {
                    return false
                }
                return isNetwork
    ...
    static var allDirectories: [URL] {
        ...
        if let googleDrive = googleDriveDirectory {
            dirs.append(googleDrive)
        }
        dirs.append(contentsOf: networkDrives)
        return dirs
    }
    ...
    private static func cloudStorageDirectory(containing name: String, startsWith: Bool = false) -> URL? {
        ...
        do {
            ...
            if match == nil {
                log.error("\(name) folder not found in ~/Library/CloudStorage")
            }
            return match
        } catch {
            log.error("Failed to access CloudStorage for \(name): \(error.localizedDescription)")
            return nil
        }
    }
}
